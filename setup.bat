...
if not exist "%PROJ_DIR%modem-manager\index.js" (
    ...
    echo [*] Checking for C:\proxy directory...
    if exist "C:\proxy\modem-manager\index.js" (
        ...
        powershell -NoProfile -ExecutionPolicy Bypass -Command ^
            ...
            "    git clone https://github.com/sammysam254/proxy.git $target;" ^
            "    if (Test-Path \"$target\modem-manager\index.js\") { $done = $true };" ^
            "  }" ^
            "} catch {};" ^
            "if (-not $done) {" ^
            "  Write-Host '[*] Downloading repository archive from GitHub...';" ^
            "  $zipUrl = 'https://github.com/sammysam254/proxy/archive/refs/heads/main.zip';" ^
            "  $zipPath = Join-Path $env:TEMP 'proxy-main.zip';" ^
            "  $tempExtract = Join-Path $env:TEMP 'proxy-extract';" ^
            "  [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 -bor [Net.SecurityProtocolType]::Tls13;" ^
            "  Invoke-WebRequest -Uri $zipUrl -OutFile $zipPath -UseBasicParsing;" ^
            "  if (Test-Path $tempExtract) { Remove-Item -Recurse -Force $tempExtract };" ^
            "  Expand-Archive -Path $zipPath -DestinationPath $tempExtract -Force;" ^
            "  Copy-Item -Path \"$tempExtract\proxy-main\*\" -Destination $target -Recurse -Force;" ^
            "  Remove-Item -Recurse -Force $zipPath, $tempExtract -ErrorAction SilentlyContinue;" ^
            "};"
...
if not exist "%PROJ_DIR%modem-manager\bin\platform-tools\adb.exe" (
    echo [*] ADB platform tools missing. Downloading official Android platform tools...
    powershell -NoProfile -ExecutionPolicy Bypass -Command ^
        "$targetDir = '%PROJ_DIR%modem-manager\bin\platform-tools';" ^
        "New-Item -ItemType Directory -Path '%PROJ_DIR%modem-manager\bin' -Force | Out-Null;" ^
        "$zipFile = Join-Path $env:TEMP 'platform-tools.zip';" ^
        "$tempDir = Join-Path $env:TEMP 'adb_temp';" ^
        "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 -bor [Net.SecurityProtocolType]::Tls13;" ^
        ...
        "Copy-Item -Path \"$tempDir\platform-tools\*\" -Destination $targetDir -Recurse -Force;" ^
        "Remove-Item -Recurse -Force $zipFile, $tempDir -ErrorAction SilentlyContinue;"
...
:: Write embedded fallback key if not present in project
if not exist "%PROJ_DIR%modem-manager\keys\proxicell_tunnel" (
    (
